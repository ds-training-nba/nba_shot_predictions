...
def fix_action_type_target_leak(df, df_train = None):
    """
    ...
    :param
    df: the df to fix, df_train: another dataframe to provide the options from
    :return: df
    """
    if df_train is None:
        df_train = df
    ...
    for prefix in action_type_prefixes_to_fix:
        ...
        sub_types[action_type_to_fix] = df_train[(df_train['MAIN_ACTION_TYPE'] == prefix) & (df_train['ACTION_TYPE'] != action_type_to_fix) ]['ACTION_TYPE'].unique()
    ...
    def fix_action_type(val):
        if val in action_types_to_fix:
            ...
            return rng.choice(possible_types)
        else:
            return val
